      ...
      if (currentUser) {
        try {
          ...
          if (error?.code === 'unavailable' || error?.message?.includes('offline')) {
            console.warn("Currently offline: could not fetch user role.");
          } else {
      ...
      <div className="p-4 border-t border-slate-800 mt-auto">
        <div className="flex items-center gap-3 glass-panel p-3 rounded-lg mb-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-slate-200 truncate">{user?.displayName || "Sentinel User"}</p>
            <div className="flex items-center gap-1 mt-0.5">
              {userRole === "admin" && <Shield className="h-3 w-3 text-indigo-400" />}
